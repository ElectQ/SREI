...
通过这个设计选择，SREI 的 loader 最终只有 5560 字节——比 sRDI 的 loader 还小，尽管 ELF 的复杂度远高于 PE。
...
运行阶段由 `loader.c` 编译生成，它是一段约 5560 字节的位置无关代码。
...
2. **计算偏移**：Bootstrap 知道自身大小（68 字节），也知道 Loader 紧随其后。通过 `rax + 68` 得到 Loader 的起始地址，通过 `rax + 68 + 5560` 得到 llbin 的起始地址。llbin 的长度存储在 llbin 头部中，紧随 llbin 之后的就是 UserData。
...
1. **更小的 loader**：5560 字节 vs sRDI 的 ~8KB，尽管 ELF 比 PE 复杂得多。
...
**关于 Rust cdylib**：Rust 的 `crate-type = ["cdylib"]` 产出的 `.so` 文件完全满足上述三个条件。它使用 `-fPIC` 编译，包含完整的重定位信息，并且通过 `#[no_mangle] pub extern "C" fn` 导出 C 兼容的函数。Rust cdylib 的体积通常很小（相比 Go），没有运行时 GC，非常适合作为 SREI 的目标。实测验证：Rust cdylib 已通过 SREI 全部三种加载模式（dlsym 参数、自解析、shellcode）的验证，包括 `no_std`（6.6KB，无运行时依赖）和 `std`（409KB，含 TLS `thread_local!`、`catch_unwind` 异常捕获、`format!` 格式化）两种配置，所有功能均正常工作。
