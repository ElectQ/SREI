...
在实际使用场景中，SREI 的 shellcode 由四部分拼接而成：Bootstrap（68 字节）+ Loader（5560 字节）+ llbin 数据 + user_data。当 Loader 完成所有加载工作后，它会计算目标导出函数名的 ROTR13 哈希，在 export 表中查找匹配的条目，获取函数地址并调用，传入 user_data 的指针和长度。user_data 是任意字节序列，嵌入在 shellcode 尾部，调用时以 `(const void *data, uint32_t len)` 的形式传递。这在渗透场景中非常实用——可以将命令、配置或另一个 payload 作为 user_data 嵌入。
